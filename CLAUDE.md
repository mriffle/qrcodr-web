...
2. **`QrStyle` carries v2 fields with v1 defaults.** `moduleShape` (`square`/`rounded`/`dot`/`horizontal-pill`/`vertical-pill`) and `centerIcon` are implemented; `canvasShape` is still a declared-but-unimplemented v2 hook (`'square'` only). The rendering pipeline already accepts it; v2 swaps the implementation, not the type. Pill modes fuse adjacent on-cells along one axis into capsules with a small cross-axis gap (`PILL_GAP`); reserved cells (finder/timing/alignment) stay square for every non-`square` shape.
